...
class IsEmptyTraversableTest extends AbstractMatcherTest
{
    ...
    {
        $this->assertMatches(emptyTraversable(), new \ArrayObject([ ]), 'an empty traversable');
    }


    public function testEmptyMatcherDoesNotMatchWhenNotEmpty()
    {
        $this->assertDoesNotMatch(emptyTraversable(), new \ArrayObject([ 1, 2, 3 ]), 'a non-empty traversable');
    }
    ...
    {
        $this->assertDoesNotMatch(nonEmptyTraversable(), new \ArrayObject([ ]), 'an empty traversable');
    }


    public function testNonEmptyMatchesWhenNotEmpty()
    {
        $this->assertMatches(nonEmptyTraversable(), new \ArrayObject([ 1, 2, 3 ]), 'a non-empty traversable');
    }
