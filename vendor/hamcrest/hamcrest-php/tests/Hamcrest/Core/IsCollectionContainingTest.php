...
class IsCollectionContainingTest extends \Hamcrest\AbstractMatcherTest
{
    ...
    public function testMatchesACollectionThatContainsAnElementMatchingTheGivenMatcher()
    {
        ...
        $this->assertMatches($itemMatcher, [ 'a', 'b', 'c' ], "should match list that contains 'a'");
    }
    ...
    public function testDoesNotMatchCollectionThatDoesntContainAnElementMatchingTheGivenMatcher()
    {
        ...
        $this->assertDoesNotMatch($matcher1, [ 'b', 'c' ], "should not match list that doesn't contain 'a'");

        $matcher2 = hasItem(equalTo('a'));
        $this->assertDoesNotMatch($matcher2, [ ], 'should not match the empty list');
    }
    ...
    public function testMatchesAllItemsInCollection()
    {
        ...
        $this->assertMatches($matcher1, [ 'a', 'b', 'c' ], 'should match list containing all items');

        $matcher2 = hasItems('a', 'b', 'c');
        $this->assertMatches($matcher2, [ 'a', 'b', 'c' ], 'should match list containing all items (without matchers)');

        $matcher3 = hasItems(equalTo('a'), equalTo('b'), equalTo('c'));
        $this->assertMatches($matcher3, [ 'c', 'b', 'a' ], 'should match list containing all items in any order');

        $matcher4 = hasItems(equalTo('a'), equalTo('b'), equalTo('c'));
        $this->assertMatches($matcher4, [ 'e', 'c', 'b', 'a', 'd' ],
            'should match list containing all items plus others');

        $matcher5 = hasItems(equalTo('a'), equalTo('b'), equalTo('c'));
        $this->assertDoesNotMatch($matcher5, [ 'e', 'c', 'b', 'd' ], // 'a' missing
