...
class PHP_CodeCoverage_Report_XML_File_Unit
{
    ...
    public function setPackage($full, $package, $sub, $category)
    {
        ...
        if ( ! $node) {
            $node = $this->contextNode->appendChild($this->contextNode->ownerDocument->createElementNS('http://schema.phpunit.de/coverage/1.0',
                'package'));
        }
    ...
    {
        $node = $this->contextNode->getElementsByTagNameNS('http://schema.phpunit.de/coverage/1.0',
            'namespace')->item(0);
