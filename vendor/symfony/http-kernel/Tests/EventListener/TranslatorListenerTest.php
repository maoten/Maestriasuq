...
class TranslatorListenerTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->translator->expects($this->once())->method('setLocale')->with($this->equalTo('fr'));
    ...
    {
        $this->translator->expects($this->at(0))->method('setLocale')->will($this->throwException(new \InvalidArgumentException()));
        $this->translator->expects($this->at(1))->method('setLocale')->with($this->equalTo('en'));
    ...
    {
        $this->requestStack->expects($this->any())->method('getParentRequest')->will($this->returnValue($request));
    }
}
