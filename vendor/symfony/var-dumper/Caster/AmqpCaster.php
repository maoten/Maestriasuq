...
class AmqpCaster
{
    ...
    public static function castExchange(\AMQPExchange $c, array $a, Stub $stub, $isNested)
    {
        ...
        $a += [
            $prefix . 'name'       => $c->getName(),
            $prefix . 'flags'      => self::extractFlags($c->getFlags()),
            $prefix . 'type'       => isset( self::$exchangeTypes[$c->getType()] ) ? new ConstStub(self::$exchangeTypes[$c->getType()],
                $c->getType()) : $c->getType(),
            $prefix . 'arguments'  => $c->getArguments(),
            $prefix . 'channel'    => $c->getChannel(),
            $prefix . 'connection' => $c->getConnection(),
    ...
    public static function castEnvelope(\AMQPEnvelope $c, array $a, Stub $stub, $isNested, $filter = 0)
    {
        ...
        $a += [
            ...
            $prefix . 'deliveryMode'    => new ConstStub($c->getDeliveryMode() . ( 2 === $c->getDeliveryMode() ? ' (persistent)' : ' (non-persistent)' ),
                $c->getDeliveryMode()),
