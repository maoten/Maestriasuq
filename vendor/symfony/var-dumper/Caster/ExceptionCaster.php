...
class ExceptionCaster
{
    ...
    public static function castThrowingCasterException(ThrowingCasterException $e, array $a, Stub $stub, $isNested)
    {
        ...
        if (isset( $a[$xPrefix . 'previous'], $a[$xPrefix . 'trace'] )) {
            ...
            $a[$xPrefix . 'trace'] = new TraceStub($b[$xPrefix . 'trace'], false, 0,
                -1 - count($a[$xPrefix . 'trace']->value));
        }
    ...
    public static function castTraceStub(TraceStub $trace, array $a, Stub $stub, $isNested)
    {
        ...
        for ($j += $trace->numberingOffset - $i++; isset( $frames[$i] ); ++$i, --$j) {
            ...
            $a[Caster::PREFIX_VIRTUAL . $j . '. ' . $call . $lastCall] = new FrameStub([
                ...
                ] + $frames[$i - 1], $trace->keepArgs, true);
            ...
            if (file_exists($f['file']) && 0 <= self::$srcContext) {
                ...
                if ( ! empty( $f['class'] ) && is_subclass_of($f['class'],
                    ...
                    try {
                        ...
                        $templateSrc  = explode("\n", method_exists($template,
                            'getSource') ? $template->getSource() : $template->getEnvironment()->getLoader()->getSource($templateName));
                        $templateInfo = $template->getDebugInfo();
                        if (isset( $templateInfo[$f['line']] )) {
                            $src[$templateName . ':' . $templateInfo[$f['line']]] = self::extractSource($templateSrc,
                                $templateInfo[$f['line']], self::$srcContext);
                        }
