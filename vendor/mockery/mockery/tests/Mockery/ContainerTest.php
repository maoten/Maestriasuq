...
class ContainerTest extends MockeryTestCase
{
    ...
    public function testGetKeyOfDemeterMockShouldReturnKeyWhenMatchingMock()
    {
        ...
        $this->assertRegExp('/Mockery_(\d+)__demeter_foo/', $this->container->getKeyOfDemeterMockFor('foo'));
    }
    ...
    {
        Mockery::getConfiguration()->setInternalClassMethodParamMap('DateTime', 'modify', [ '&$string' ]);
    ...
    public function testCanOverrideExpectedParametersOfExtensionPHPClassesToPreserveRefs()
    {
        ...
        Mockery::getConfiguration()->setInternalClassMethodParamMap('MongoCollection', 'insert',
            [ '&$data', '$options' ]);
        // @ used to avoid E_STRICT for incompatible signature
    ...
    {
        $m = Mockery::mock('StdClass')->shouldReceive('get')->andReturn(false)->getMock();
        $m->get();
        Mockery::close();
    }
    ...
    public function testMockeryShouldDistinguishBetweenConstructorParamsAndClosures()
    {
        ...
        $mock = $this->container->mock('MockeryTest_ClassMultipleConstructorParams[dave]', [ &$obj, 'foo' ]);
    }
