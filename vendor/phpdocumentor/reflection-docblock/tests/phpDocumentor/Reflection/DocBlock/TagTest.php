...
class TagTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testTagHandlerUnregistration()
    {
        ...
        $this->assertInstanceOf($currentHandler, $tagPreUnreg);
        $this->assertInstanceOf(__NAMESPACE__ . '\Tag', $tagPreUnreg);

        Tag::registerTagHandler('var', null);

        $tagPostUnreg = Tag::createInstance('@var mixed');
        $this->assertNotInstanceOf($currentHandler, $tagPostUnreg);
        $this->assertInstanceOf(__NAMESPACE__ . '\Tag', $tagPostUnreg);
...
TAG_HANDLER
        ...
        $this->assertNotInstanceOf($currentHandler, $tagPostReg);
        $this->assertInstanceOf(__NAMESPACE__ . '\Tag', $tagPostReg);
        $this->assertInstanceOf('\MyTagHandler', $tagPostReg);
    ...
    public function testNamespacedTagHandlerCorrectRegistration()
    {
        ...
        $this->assertInstanceOf(__NAMESPACE__ . '\Tag', $tagPreReg);
        $this->assertNotInstanceOf('\MyTagHandler', $tagPreReg);

        $this->assertTrue(Tag::registerTagHandler('\MyNamespace\MyTag', '\MyTagHandler'));

        $tagPostReg = Tag::createInstance('@T something',
            new DocBlock('', new Context('', [ 'T' => '\MyNamespace\MyTag' ])));
        $this->assertInstanceOf(__NAMESPACE__ . '\Tag', $tagPostReg);
        $this->assertInstanceOf('\MyTagHandler', $tagPostReg);

        $this->assertTrue(Tag::registerTagHandler('\MyNamespace\MyTag', null));
    }
    ...
    public function testNamespacedTagHandlerIncorrectRegistration()
    {
        ...
        $this->assertFalse(Tag::registerTagHandler('MyNamespace\MyTag', '\MyTagHandler'));

        $tagPostReg = Tag::createInstance('@T something',
            new DocBlock('', new Context('', [ 'T' => '\MyNamespace\MyTag' ])));
        $this->assertInstanceOf(__NAMESPACE__ . '\Tag', $tagPostReg);
        $this->assertNotInstanceOf('\MyTagHandler', $tagPostReg);
    }
