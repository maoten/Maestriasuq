...
class Validator implements ValidatorContract
{
    ...
    protected function validate($attribute, $rule)
    {
        ...
        if (( $keys = $this->getNumericKeys($attribute) ) && $this->dependsOnOtherFields($rule)) {
    ...
    {
        return $this->presentOrRuleIsImplicit($rule, $attribute,
            $value) && $this->passesOptionalCheck($attribute) && $this->hasNotFailedPreviousRuleIfPresenceRule($rule,
            $attribute);
    }
    ...
    protected function passesOptionalCheck($attribute)
    {
        if ($this->hasRule($attribute, [ 'Sometimes' ])) {
            return array_key_exists($attribute, Arr::dot($this->data)) || in_array($attribute,
                array_keys($this->data)) || array_key_exists($attribute, $this->files);
        }
    ...
    protected function doReplacements($message, $attribute, $rule, $parameters)
    {
        ...
        $message = str_replace([ ':ATTRIBUTE', ':Attribute', ':attribute' ],
            [ Str::upper($value), Str::ucfirst($value), $value ], $message);
