...
class JsonFormatterTest extends TestCase
{
    ...
    public function testDefFormatWithPreviousException()
    {
        ...
        if (version_compare(PHP_VERSION, '5.4.0', '>=')) {
            $pathPrevious  = substr(json_encode($exception->getPrevious()->getFile(),
                JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE), 1, -1);
            $pathException = substr(json_encode($exception->getFile(), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE),
                1, -1);
        } else {
