...
class Swift_Mime_SimpleMimeEntity implements Swift_Mime_MimeEntity
{
    ...
    private function _childSortAlgorithm($a, $b)
    {
        ...
        foreach ($types as $type) {
            $typePrefs[] = ( array_key_exists($type,
                $this->_alternativePartOrder) ) ? $this->_alternativePartOrder[$type] : ( max($this->_alternativePartOrder) + 1 );
        }
    ...
    {
        if ( ! preg_match('/^' . $this->_grammar->getDefinition('id-left') . '@' . $this->_grammar->getDefinition('id-right') . '$/D',
            $id)
        ) {
