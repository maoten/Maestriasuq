...
class Swift_Plugins_LoggerPluginTest extends \SwiftMailerTestCase
{
    ...
    public function testLoggerDelegatesAddingEntries()
    {
        ...
        $logger->expects($this->once())->method('add')->with('foo');
    ...
    {
        $logger = $this->_createLogger();
        $logger->expects($this->once())->method('dump')->will($this->returnValue('foobar'));
    ...
    public function testCommandIsSentToLogger()
    {
        ...
        $logger = $this->_createLogger();
        $logger->expects($this->once())->method('add')->with($this->regExp('~foo\r\n~'));
    ...
    public function testResponseIsSentToLogger()
    {
        ...
        $logger = $this->_createLogger();
        $logger->expects($this->once())->method('add')->with($this->regExp('~354 Go ahead\r\n~'));
    ...
    public function testTransportBeforeStartChangeIsSentToLogger()
    {
        ...
        $logger = $this->_createLogger();
        $logger->expects($this->once())->method('add')->with($this->anything());
    ...
    private function _createTransportChangeEvent()
    {
        ...
        $evt->expects($this->any())->method('getSource')->will($this->returnValue($this->_createTransport()));
    ...
    {
        $evt = $this->getMockBuilder('Swift_Events_TransportExceptionEvent')->disableOriginalConstructor()->getMock();
