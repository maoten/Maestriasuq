...
class Swift_Encoder_Base64EncoderTest extends \PHPUnit_Framework_TestCase
{
    ...
    */
    ...
    public function testInputOutputRatioIs3to4Bytes()
    {
        ...
        $this->assertEquals('MTIz', $this->_encoder->encodeString('123'),
            '%s: 3 bytes of input should yield 4 bytes of output');
        $this->assertEquals('MTIzNDU2', $this->_encoder->encodeString('123456'),
            '%s: 6 bytes in input should yield 8 bytes of output');
        $this->assertEquals('MTIzNDU2Nzg5', $this->_encoder->encodeString('123456789'),
    ...
    public function testPadLength()
    {
       ...
       */
        ...
        for ($i = 0; $i < 30; ++$i) {
            ...
            $this->assertRegExp('~^[a-zA-Z0-9/\+]{2}==$~', $this->_encoder->encodeString($input),
                '%s: A single byte should have 2 bytes of padding');
        }

        for ($i = 0; $i < 30; ++$i) {
            $input = pack('C*', rand(0, 255), rand(0, 255));
            $this->assertRegExp('~^[a-zA-Z0-9/\+]{3}=$~', $this->_encoder->encodeString($input),
                '%s: Two bytes should have 1 byte of padding');
        }

        for ($i = 0; $i < 30; ++$i) {
            $input = pack('C*', rand(0, 255), rand(0, 255), rand(0, 255));
            $this->assertRegExp('~^[a-zA-Z0-9/\+]{4}$~', $this->_encoder->encodeString($input),
    ...
    public function testMaximumLineLengthIs76Characters()
    {
        ...
        $this->assertEquals($output, $this->_encoder->encodeString($input),
    ...
    public function testMaximumLineLengthCanBeSpecified()
    {
        ...
        $this->assertEquals($output, $this->_encoder->encodeString($input, 0, 50),
    ...
    public function testFirstLineLengthCanBeDifferent()
    {
        ...
        $this->assertEquals($output, $this->_encoder->encodeString($input, 19),
