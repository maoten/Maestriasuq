...
class Swift_Transport_Esmtp_AuthHandlerTest extends \SwiftMailerTestCase
{
    ...
    public function testAuthenticatorsAreCalledAccordingToParamsAfterEhlo()
    {
        ...
        $a1->shouldReceive('authenticate')->never()->with($this->_agent, 'jack', 'pass');
        $a2->shouldReceive('authenticate')->once()->with($this->_agent, 'jack', 'pass')->andReturn(true);
    ...
    public function testAuthenticatorsAreNotUsedIfNoUsernameSet()
    {
        ...
        $a1->shouldReceive('authenticate')->never()->with($this->_agent, 'jack', 'pass');
        $a2->shouldReceive('authenticate')->never()->with($this->_agent, 'jack', 'pass')->andReturn(true);
    ...
    public function testSeveralAuthenticatorsAreTriedIfNeeded()
    {
        ...
        $a1->shouldReceive('authenticate')->once()->with($this->_agent, 'jack', 'pass')->andReturn(false);
        $a2->shouldReceive('authenticate')->once()->with($this->_agent, 'jack', 'pass')->andReturn(true);
    ...
    public function testFirstAuthenticatorToPassBreaksChain()
    {
        ...
        $a1->shouldReceive('authenticate')->once()->with($this->_agent, 'jack', 'pass')->andReturn(false);
        $a2->shouldReceive('authenticate')->once()->with($this->_agent, 'jack', 'pass')->andReturn(true);
        $a3->shouldReceive('authenticate')->never()->with($this->_agent, 'jack', 'pass');
    ...
    private function _createMockAuthenticator($type)
    {
        ...
        $authenticator->shouldReceive('getAuthKeyword')->zeroOrMoreTimes()->andReturn($type);
