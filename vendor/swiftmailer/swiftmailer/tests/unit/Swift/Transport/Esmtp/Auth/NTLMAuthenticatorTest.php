...
class Swift_Transport_Esmtp_Auth_NTLMAuthenticatorTest extends \SwiftMailerTestCase
{
    ...
    public function testLMv1Generator()
    {
        ...
        $lmv1Result = $this->_invokePrivateMethod('createLMPassword', $login,
            [ $password, $this->hex2bin($challenge) ]);
    ...
    public function testLMv2Generator()
    {
        ...
        $lmv2Result = $this->_invokePrivateMethod('createLMv2Password', $login,
            [ $password, $username, $domain, $this->hex2bin($challenge), $this->hex2bin('ffffff0011223344') ]);
    ...
    public function testMessage3v1Generator()
    {
        ...
        $message3 = $this->_invokePrivateMethod('createMessage3', $login,
            [ $domain, $username, $workstation, $this->hex2bin($lmResponse), $this->hex2bin($ntlmResponse) ]);
    ...
    public function testGetDomainAndUsername()
    {
        ...
        $login = $this->_getAuthenticator();
        list( $domain, $user ) = $this->_invokePrivateMethod('getDomainAndUsername', $login, [ $username ]);
    ...
    public function testSuccessfulAuthentication()
    {
        ...
        $agent->shouldReceive('executeCommand')->once()->with('AUTH NTLM ' . base64_encode($this->_invokePrivateMethod('createMessage1',
                $ntlm)) . "\r\n",
        ...
        $agent->shouldReceive('executeCommand')->once()->with(base64_encode($this->_invokePrivateMethod('createMessage3',
                $ntlm, [
                    $domain,
                    $username,
    ...
    public function testAuthenticationFailureSendRsetAndReturnFalse()
    {
        ...
        $agent = $this->_getAgent();
        $agent->shouldReceive('executeCommand')->once()->with('AUTH NTLM ' . base64_encode($this->_invokePrivateMethod('createMessage1',
                $ntlm)) . "\r\n", [ 334 ])->andThrow(new Swift_TransportException(''));
        $agent->shouldReceive('executeCommand')->once()->with("RSET\r\n", [ 250 ]);
