...
class Swift_Mime_SimpleHeaderFactoryTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $header = $this->_factory->createMailboxHeader('X-Foo', [ 'foo@bar' => 'FooBar' ]);
    ...
    {
        $header = $this->_factory->createParameterizedHeader('X-Foo', 'bar', [ 'zip' => 'button' ]);
    ...
    public function testCharsetChangeNotificationNotifiesEncoders()
    {
        ...
        $paramEncoder->expects($this->once())->method('charsetChanged')->with('utf-8');
    ...
    {
        return new Swift_Mime_SimpleHeaderFactory($encoder ? $encoder : $this->_createHeaderEncoder(),
            $paramEncoder ? $paramEncoder : $this->_createParamEncoder(), new Swift_Mime_Grammar());
    }
