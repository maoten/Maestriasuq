...
abstract class Swift_Mime_AbstractMimeEntityTest extends \SwiftMailerTestCase
{
    ...
    public function testContentTypeIsSetInHeader()
    {
        ...
        $ctype->shouldReceive('setFieldBodyModel')->once()->with('image/jpeg');
        $ctype->shouldReceive('setFieldBodyModel')->zeroOrMoreTimes()->with(\Mockery::not('image/jpeg'));
    ...
    public function testContentTypeHeaderIsAddedIfNoneSet()
    {
        ...
        $headers->shouldReceive('addParameterizedHeader')->once()->with('Content-Type', 'image/jpeg');
    ...
    {
        $headers = $this->_createHeaderSet([ ], false);
        $headers->shouldReceive('addParameterizedHeader')->once()->with('Content-Type', 'text/html');
        $headers->shouldReceive('addParameterizedHeader')->zeroOrMoreTimes();
    ...
    {
        $encoder  = $this->_createEncoder('base64');
        $encoding = $this->_createHeader('Content-Transfer-Encoding', '8bit', [ ], false);
        $headers  = $this->_createHeaderSet([
            'Content-Transfer-Encoding' => $encoding,
        ]);
        $encoding->shouldReceive('setFieldBodyModel')->once()->with('base64');
    ...
    {
        $headers = $this->_createHeaderSet([ ], false);
        $headers->shouldReceive('addTextHeader')->once()->with('Content-Transfer-Encoding', 'something');
    ...
    public function testIdIsSetInHeader()
    {
        ...
        $cid->shouldReceive('setFieldBodyModel')->once()->with('foo@bar');
    ...
    {
        $headers = $this->_createHeaderSet([ ], false);
        $headers->shouldReceive('addTextHeader')->once()->with('Content-Description', 'whatever');
        $headers->shouldReceive('addTextHeader')->zeroOrMoreTimes();
    ...
    public function testEncoderIsUsedForStringGeneration()
    {
        ...
        $encoder->expects($this->once())->method('encodeString')->with('blah');
    ...
    {
        $encoder = $this->_createEncoder('base64', false);
        $encoder->expects($this->once())->method('encodeString')->with('blah', 0, 65);
    ...
    {
        $headers = $this->_createHeaderSet([ ], false);
        $headers->shouldReceive('toString')->once()->andReturn("Content-Type: text/plain; charset=utf-8\r\n" . "X-MyHeader: foobar\r\n");
    ...
    public function testBoundaryCanBeRetrieved()
    {
     ...
     bcharsnospace := DIGIT / ALPHA / "'" / "(" / ")" /
        ...
        */
        ...
        $entity = $this->_createEntity($this->_createHeaderSet(), $this->_createEncoder(), $this->_createCache());
        $this->assertRegExp('/^[a-zA-Z0-9\'\(\)\+_\-,\.\/:=\?\ ]{0,69}[a-zA-Z0-9\'\(\)\+_\-,\.\/:=\?]$/D',
            $entity->getBoundary());
    }
    ...
    {
        $entity        = $this->_createEntity($this->_createHeaderSet(), $this->_createEncoder(),
            $this->_createCache());
    ...
    public function testAddingChildrenGeneratesBoundaryInHeaders()
    {
        ...
        $cType = $this->_createHeader('Content-Type', 'text/plain', [ ], false);
        $cType->shouldReceive('setParameter')->once()->with('boundary', \Mockery::any());
    ...
    public function testChildrenOfLevelAttachmentAndLessCauseMultipartMixed()
    {
        for ($level = Swift_Mime_MimeEntity::LEVEL_MIXED; $level > Swift_Mime_MimeEntity::LEVEL_TOP; $level /= 2) {
            ...
            $cType = $this->_createHeader('Content-Type', 'text/plain', [ ], false);
            $cType->shouldReceive('setFieldBodyModel')->once()->with('multipart/mixed');
    ...
    public function testChildrenOfLevelRelatedAndLessCauseMultipartRelated()
    {
        for ($level = Swift_Mime_MimeEntity::LEVEL_RELATED; $level > Swift_Mime_MimeEntity::LEVEL_ALTERNATIVE; $level /= 2) {
            $child = $this->_createChild($level);
            $cType = $this->_createHeader('Content-Type', 'text/plain', [ ], false);
            $cType->shouldReceive('setFieldBodyModel')->once()->with('multipart/related');
            $cType->shouldReceive('setFieldBodyModel')->zeroOrMoreTimes();
    ...
    public function testHighestLevelChildDeterminesContentType()
    {
        ...
        foreach ($combinations as $combination) {
            ...
            $cType = $this->_createHeader('Content-Type', 'text/plain', [ ], false);
            $cType->shouldReceive('setFieldBodyModel')->once()->with($combination['type']);
     ...
     */
        ...
        $headers->shouldReceive('toString')->zeroOrMoreTimes()->andReturn("Content-Type: multipart/alternative; boundary=\"xxx\"\r\n");
    ...
    public function testMixingLevelsIsHierarchical()
    {
        ...
        $headers->shouldReceive('toString')->zeroOrMoreTimes()->andReturn("Content-Type: multipart/mixed; boundary=\"xxx\"\r\n");
        $headers->shouldReceive('newInstance')->zeroOrMoreTimes()->andReturn($newHeaders);
        $newHeaders->shouldReceive('toString')->zeroOrMoreTimes()->andReturn("Content-Type: multipart/alternative; boundary=\"yyy\"\r\n");
    ...
    public function testSettingEncoderNotifiesChildren()
    {
        ...
        $child->shouldReceive('encoderChanged')->once()->with($encoder);
    ...
    {
        $child = $this->_createChild(0, '', false);
        $child->shouldReceive('charsetChanged')->once()->with('windows-874');
    ...
    {
        $headers = $this->_createHeaderSet([ ], false);
        $headers->shouldReceive('toString')->zeroOrMoreTimes()->andReturn("Content-Type: text/plain; charset=utf-8\r\n");

        $cache = $this->_createCache(false);
        $cache->shouldReceive('hasKey')->once()->with(\Mockery::any(), 'body')->andReturn(true);
        $cache->shouldReceive('getString')->once()->with(\Mockery::any(), 'body')->andReturn("\r\ncache\r\ncache!");
    ...
    public function testBodyIsClearedFromCacheIfNewBodySet()
    {
        ...
        $cache->shouldReceive('clearKey')->once()->with(\Mockery::any(), 'body');
    ...
    public function testBodyIsReadFromCacheWhenUsingToByteStreamIfPresent()
    {
        ...
        $cache = $this->_createCache(false);
        $cache->shouldReceive('hasKey')->once()->with(\Mockery::any(), 'body')->andReturn(true);
        $cache->shouldReceive('exportToByteStream')->once()->with(\Mockery::any(), 'body', $is);
    ...
    public function testBodyIsAddedToCacheWhenUsingToByteStream()
    {
        ...
        $cache = $this->_createCache(false);
        $cache->shouldReceive('hasKey')->once()->with(\Mockery::any(), 'body')->andReturn(false);
        $cache->shouldReceive('getInputByteStream')->once()->with(\Mockery::any(), 'body');
    ...
    public function testFluidInterface()
    {
        ...
        $this->assertSame($entity,
            $entity->setContentType('text/plain')->setEncoder($this->_createEncoder())->setId('foo@bar')->setDescription('my description')->setMaxLineLength(998)->setBody('xx')->setBoundary('xyz')->setChildren([ ]));
    }
