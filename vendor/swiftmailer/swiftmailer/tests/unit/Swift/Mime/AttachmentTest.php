...
class Swift_Mime_AttachmentTest extends Swift_Mime_AbstractMimeEntityTest
{
    ...
    {
        $attachment = $this->_createAttachment($this->_createHeaderSet(), $this->_createEncoder(),
            $this->_createCache());
    ...
    {
        $disposition = $this->_createHeader('Content-Disposition', 'attachment', [ ], false);
        $disposition->shouldReceive('setFieldBodyModel')->once()->with('inline');
    ...
    public function testDispositionIsAddedIfNonePresent()
    {
        ...
        $headers->shouldReceive('addParameterizedHeader')->once()->with('Content-Disposition', 'inline');
    ...
    {
        $headers = $this->_createHeaderSet([ ], false);
        $headers->shouldReceive('addParameterizedHeader')->once()->with('Content-Disposition', 'attachment');
        $headers->shouldReceive('addParameterizedHeader')->zeroOrMoreTimes();
    ...
    {
        $cType = $this->_createHeader('Content-Type', '', [ ], false);
        $cType->shouldReceive('setFieldBodyModel')->once()->with('application/octet-stream');
     ...
     */
        ...
        $disposition = $this->_createHeader('Content-Disposition', 'attachment', [ 'filename' => 'foo.txt' ]);
        $attachment  = $this->_createAttachment($this->_createHeaderSet([
            'Content-Disposition' => $disposition,
        ]), $this->_createEncoder(), $this->_createCache());
    ...
    {
        $disposition = $this->_createHeader('Content-Disposition', 'attachment', [ 'filename' => 'foo.txt' ], false);
        $disposition->shouldReceive('setParameter')->once()->with('filename', 'bar.txt');
     ...
     */
        ...
        $cType->shouldReceive('setParameter')->once()->with('name', 'bar.txt');
     ...
     */
        ...
        $disposition = $this->_createHeader('Content-Disposition', 'attachment', [ 'size' => 1234 ]);
        $attachment  = $this->_createAttachment($this->_createHeaderSet([
            'Content-Disposition' => $disposition,
        ]), $this->_createEncoder(), $this->_createCache());
    ...
    {
        $disposition = $this->_createHeader('Content-Disposition', 'attachment', [ ], false);
        $disposition->shouldReceive('setParameter')->once()->with('size', 12345);
        $disposition->shouldReceive('setParameter')->zeroOrMoreTimes();
    ...
    public function testFilnameCanBeReadFromFileStream()
    {
        ...
        $disposition = $this->_createHeader('Content-Disposition', 'attachment', [ 'filename' => 'foo.txt' ], false);
        $disposition->shouldReceive('setParameter')->once()->with('filename', 'file.ext');
    ...
    public function testContentTypeCanBeSetViaSetFile()
    {
        ...
        $ctype = $this->_createHeader('Content-Type', 'text/plain', [ ], false);
        $ctype->shouldReceive('setFieldBodyModel')->once()->with('text/html');
        $ctype->shouldReceive('setFieldBodyModel')->zeroOrMoreTimes();
    ...
    public function XtestContentTypeCanBeLookedUpFromCommonListIfNotProvided()
    {
        ...
        $disposition = $this->_createHeader('Content-Disposition', 'attachment', [ 'filename' => 'foo.zip' ], false);
        $disposition->shouldReceive('setParameter')->once()->with('filename', 'file.zip');

        $ctype = $this->_createHeader('Content-Type', 'text/plain', [ ], false);
        $ctype->shouldReceive('setFieldBodyModel')->once()->with('application/zip');
        $ctype->shouldReceive('setFieldBodyModel')->zeroOrMoreTimes();
