...
class Swift_CharacterReader_GenericFixedWidthReaderTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testValidationValueIsBasedOnOctetCount()
    {
        ...
        $this->assertSame(1, $reader->validateByteSequence([ 0x01, 0x02, 0x03 ], 3)); //3 octets
        ...
        $this->assertSame(0, $reader->validateByteSequence([ 0xFE, 0x03, 0x67, 0x9A ], 4)); //All 4 octets
    }
    ...
    public function testValidationFailsIfTooManyOctets()
    {
        ...
        $this->assertSame(-1,
            $reader->validateByteSequence([ 0xFE, 0x03, 0x67, 0x9A, 0x10, 0x09, 0x85 ], 7)); //7 octets
    }
}
