...
class Swift_Smoke_AttachmentSmokeTest extends SwiftMailerSmokeTestCase
{
    ...
    public function testAttachmentSending()
    {
        ...
        $message = Swift_Message::newInstance()->setSubject('[Swift Mailer] AttachmentSmokeTest')->setFrom([ SWIFT_SMOKE_EMAIL_ADDRESS => 'Swift Mailer' ])->setTo(SWIFT_SMOKE_EMAIL_ADDRESS)->setBody('This message should contain an attached ZIP file (named "textfile.zip").' . PHP_EOL . 'When unzipped, the archive should produce a text file which reads:' . PHP_EOL . '"This is part of a Swift Mailer v4 smoke test."')->attach(Swift_Attachment::fromPath($this->_attFile));
