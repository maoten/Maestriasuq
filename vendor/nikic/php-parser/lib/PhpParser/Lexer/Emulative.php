...
class Emulative extends \PhpParser\Lexer
{
    ...
    protected function postprocessTokens()
    {
        ...
        for ($i = 0, $c = count($this->tokens); $i < $c; ++$i) {
            ...
            if ('~' === $this->tokens[$i] && isset( $this->tokens[$i + 2] ) && '~' === $this->tokens[$i + 2] && T_STRING === $this->tokens[$i + 1][0] && preg_match('(^__EMU__([A-Z]++)__(?:([A-Za-z0-9]++)__)?$)',
                    $this->tokens[$i + 1][1], $matches)
            ) {
                ...
                $this->tokens[$i][1] = preg_replace_callback('(~__EMU__([A-Z]++)__(?:([A-Za-z0-9]++)__)?~)',
                    [ $this, 'restoreContentCallback' ], $this->tokens[$i][1]);
            }
        }
    }
