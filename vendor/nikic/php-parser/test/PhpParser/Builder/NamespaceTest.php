...
class NamespaceTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testCreation()
    {
        ...
        $expected = new Stmt\Namespace_(new Node\Name('Name\Space'), [ $stmt1, $stmt2, $stmt3 ]);

        $node = $this->createNamespaceBuilder('Name\Space')->addStmt($stmt1)->addStmts([ $stmt2, $stmt3 ])->getNode();
        $this->assertEquals($expected, $node);

        $node = $this->createNamespaceBuilder(new Node\Name([ 'Name', 'Space' ]))->addStmts([
                $stmt1,
                $stmt2
            ])->addStmt($stmt3)->getNode();
        $this->assertEquals($expected, $node);
