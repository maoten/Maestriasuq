	...
	<div class="row">
		<div class="col-md-12">
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<div>
						<form class="form-horizontal" role="form" method="GET" action="{{ route('consejo.propuesta.asignar_jurados', $propuesta->id) }}" >

							@if(App\Jurado_propuesta::where('propuesta_id',$propuesta->id)->first()==null)
							<p class="text-center">Esta propuesta no tiene jurados asignados aún.<br>Antes de asignar jurados a esta propuesta revisa que hay por lo menos 3 jurados registrados.</p>
							...
							<div class="form-group">
								...
								<div class="col-md-6">
									<div class="input-group">
										...
										<select class="chosen" multiple="true" style="width:300px;" name="jurados[]">
											@foreach((App\User::where('rol','jurado')->get()) as $jurado)
											<option value="{{ $jurado->id }}">{{ $jurado->nombre }}</option>
											@endforeach 
										</select>
										<button type="submit" class="btn btn-primary" name="registrar">Asignar jurados
										</button>
							...
							@endforeach 
							<hr>
							<a class="btn btn-warning" id="camb" title="Cambiar jurados" onClick="cambiar()">cambiar jurados
							</a>
								...
								<div class="col-md-6">
									<div class="input-group">
										<select class="chosen" multiple="true" style="width:300px;" name="jurados[]">
											...
											@foreach(App\User::where('rol','jurado')->get() as $jurado)
                                            <?php $jurad=App\Jurado_propuesta::where('jurado_id',$jurado->id)->where('propuesta_id',$propuesta->id)->first() ?>
											@if($jurad!=null)
											
											<option selected="selected" value="{{ $jurado->id }}">{{ $jurado->nombre }}</option>
											@else
											<option value="{{ $jurado->id }}">{{ $jurado->nombre }}</option>
										...
										<button type="submit" class="btn btn-primary" name="registrar">Cambiar jurados
										</button>
